<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6">
    <div class="mb-4">
      <h1 class="text-2xl font-bold text-slate-900">{{ t('quote.title') }}</h1>
      <p class="text-sm text-slate-500">{{ t('quote.subtitle') }}</p>
    </div>
      ...
      <div class="lg:w-[480px] shrink-0 space-y-4">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <div><label class="label">{{ t('quote.number') }}</label><input v-model="form.invoiceNumber" class="input" /></div>
            <div><label class="label">{{ t('common.currency') }}</label>
              <select v-model="form.currency" class="input">
                <option v-for="c in currencies" :key="c" :value="c">{{ c }}</option>
              </select>
            </div>
            <div><label class="label">{{ t('quote.date') }}</label><input v-model="form.date" type="date" class="input" /></div>
            <div><label class="label">{{ t('quote.dueDate') }}</label><input v-model="form.dueDate" type="date" class="input" /></div>
          </div>
        </div>
      ...
      <div class="flex-1 min-w-0">
        <div class="sticky top-20">
          ...
          <div class="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            <DocumentPreview :form="form" :docLabel="t('quote.label')" :numberLabel="t('quote.number')" :dueDateLabel="t('quote.dueDate')" />
          </div>
        </div>
      </div>
    </div>
...
async function downloadPdf() {
  ...
  try { await exportToPdf('invoice-preview', `quote-${form.invoiceNumber}.pdf`) }
