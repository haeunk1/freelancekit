          ...
          <div>
            ...
            <div class="flex items-center gap-2 flex-wrap">
              <button v-for="c in presetColors" :key="c" @click="form.color = c" type="button"
                :style="`background:${c};width:28px;height:28px;border-radius:50%;border:3px solid ${form.color === c ? '#1e293b' : 'transparent'};transition:all 0.15s;`">
              </button>
              <label class="relative cursor-pointer" title="Custom color">
                <div class="w-7 h-7 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center hover:border-slate-500 transition-colors overflow-hidden">
                  ...
                  <svg v-if="isPreset" class="w-3.5 h-3.5 text-slate-400 absolute" fill="none" stroke="currentColor" viewBox="0 0 24 24">
